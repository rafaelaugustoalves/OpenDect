	...
	for epoch in range(0,imax):
		...
		for i in range(0,n):
			...
			if pb_out[i]<gb_out:
				...
				index=0
				for j,param in enumerate(window.StaticParams):
					if not window.ActiveParams[j]:
						window.DynamicParams[j]=param
					else:
...
def Swarmfunction(p,hist,ExpParams,Orientation,Padding_top,Padding_bottom,Crop_pct,nblocks,nblocks_z,nCycle,clength,Swcr,n,window):
	...
	for i in range(0,n):
		...
		Epsilon=0.00001
		WriteDATAfile(
			window.height,ExpParams,Orientation,
			Padding_top,Padding_bottom,Crop_pct,
			nblocks,nblocks_z,window.DynamicParams[3],
			window.DynamicParams[4],window.DynamicParams[5],
			Swcr,window.DynamicParams[6],window.DynamicParams[7],
			window.DynamicParams[0],window.DynamicParams[1],
			window.DynamicParams[2],window.DynamicParams[11],
			window.DynamicParams[10],window.DynamicParams[9],
			window.DynamicParams[8],nCycle,clength,i,
		)
	...
	for i in range(0,n):
		summary = Summary("temp/CORE_TEST-"+str(i)+".DATA")
